...
#define ALIGN_POINTER(p, align) ((uint8_t*)(((uintptr_t)(p) + (align - 1)) & ~(align - 1)))

struct HuffRange;

int Kraken_DecodeBytes(
  uint8_t **output, const uint8_t *src, const uint8_t *src_end, int *decoded_size,
  size_t output_size, bool force_memmove, uint8_t *scratch, uint8_t *scratch_end
);
...
#define COPY_64_ADD(d, s, t) _mm_storel_epi64(( \
  __m128i *)(d), _mm_add_epi8(_mm_loadl_epi64((__m128i *)(s)), _mm_loadl_epi64((__m128i *)(t))))
...
uint8_t *Kraken_ParseHeader(KrakenHeader *hdr, uint8_t *p) {
  int b = p[0];
  if ((b & 0xF) == 0xC) {
...
uint8_t *Kraken_ParseQuantumHeader(KrakenQuantumHeader *hdr, uint8_t *p, bool use_checksum) {
...
uint8_t *LZNA_ParseWholeMatchInfo(uint8_t *p, uint32_t *dist) {
...
uint8_t *LZNA_ParseQuantumHeader(
  KrakenQuantumHeader *hdr, uint8_t *p, bool use_checksum, int32_t raw_len
) {
...
bool Kraken_DecodeBytesCore(HuffReader *hr, HuffRevLut *lut) {
  ...
  if (hr->src_end - src_mid >= 4 && dst_end - dst >= 6) {
    ...
    while (dst < dst_end && src <= src_mid && src_mid <= src_end) {
      ...
      src_end_bits |= __byteswap_ulong(*(uint32_t*)src_end) << src_end_bitpos;
...
bool DecodeGolombRiceBits(uint8_t *dst, uint32_t size, uint32_t bitcount, BitReader2 *br) {
  if (bitcount == 0)
    return true;
  uint8_t *dst_end = dst + size;
  ...
  if (bitcount < 2) {
    ...
    do {
      // Read the next byte
      uint64_t bits = (uint8_t)(__byteswap_ulong(*(uint32_t*)p) >> (24 - bitpos));
      p += 1;
    ...
    do {
      ...
      uint64_t bits = (uint16_t)(__byteswap_ulong(*(uint32_t*)p) >> (16 - bitpos));
      p += 2;
      // Expand each bit into each byte of the uint64.
    ...
    do {
      ...
      uint64_t bits = (__byteswap_ulong(*(uint32_t*)p) >> (8 - bitpos)) & 0xffffff;
      p += 3;
      // Expand each bit into each byte of the uint64.
...
int32_t Huff_ReadCodeLengthsNew(BitReader *bits, uint8_t *syms, uint32_t *code_prefix) {
  ...
  if (1) {
    ...
    for (uint i = 0; i < num_symbols; i += 16) {
      ...
      // u[x] = (avg + t[x-1]) >> 2
      __m128i u = _mm_and_si128(_mm_srli_epi16(
        _mm_add_epi8(_mm_slli_si128(t, 1), avg), 2u), _mm_set1_epi8(0x3f));
      // v += u
      v = _mm_add_epi8(v, u);
      // avg += t
...
bool Huff_MakeLut(
  const uint32_t *prefix_org, const uint32_t *prefix_cur, NewHuffLut *hufflut, uint8_t *syms
) {
  uint32_t currslot = 0;
...
int Kraken_DecodeBytes_Type12(
  const uint8_t *src, size_t src_size, uint8_t *output, int output_size, int type
) {
  BitReader bits;
  ...
  if (num_arrays_in_file == 0) {
    for (int i = 0; i < array_count; i++) {
      ...
      int dec = Kraken_DecodeBytes(
        &chunk_dst, src, src_end, &decoded_size, dst_end - dst, force_memmove, scratch, scratch_end);
      if (dec < 0)
        return -1;
      dst += decoded_size;
  ...
  *total_size_out = total_size;
  ...
  if (Q & 0x8000) {
    ...
    int n = Kraken_DecodeBytes(
      &interval_indexes, src, src_end, &size_out, num_indexes, false, scratch_cur, scratch_end);
    ...
    n = Kraken_DecodeBytes(
      &interval_lenlog2, src, src_end, &size_out, lenlog2_chunksize, false, scratch_cur, scratch_end);
  ...
  for (i = 0; i + 2 <= num_lens; i += 2) {
    bits_f |= __byteswap_ulong(*(uint32_t*)f) >> (24 - bitpos_f);
    f += (bitpos_f + 7) >> 3;
    ...
    bits_f = std::rotl(bits_f | 1, numbits_f);
    bitpos_f += numbits_f - 8 * ((bitpos_f + 7) >> 3);

    bits_b = std::rotl(bits_b | 1, numbits_b);
...
int Krak_DecodeRecursive(
  const uint8_t *src, size_t src_size, uint8_t *output,
  int output_size, uint8_t *scratch, uint8_t *scratch_end) {
  const uint8_t *src_org = src;
  ...
  if (!(src[0] & 0x80)) {
    ...
    do {
      int decoded_size;
      int dec = Kraken_DecodeBytes(
        &output, src, src_end, &decoded_size, output_end - output, true, scratch, scratch_end);
      if (dec < 0)
        return -1;
      output += decoded_size;
      src += dec;
...
int Krak_DecodeRLE(
  ...
  if (src[0]) {
    ...
    int dec_size;
    int n = Kraken_DecodeBytes(
      &dst_ptr, src, src + src_size, &dec_size,
...
void SimpleSort(uint32_t *p, uint32_t *pend) {
  if (p != pend) {
    for (uint32_t *lp = p + 1, *rp; lp != pend; lp++) {
      uint32_t t = lp[0];
...
#define TANS_BACKWARD_BITS()                    \
    bits_b |= __byteswap_ulong(((uint32_t *)ptr_b)[-1]) << bitpos_b;     \
    ptr_b -= (31 - bitpos_b) >> 3;              \
    bitpos_b |= 24;
...
int Krak_DecodeTans(const uint8_t *src, size_t src_size, uint8_t *dst,
  int dst_size, uint8_t *scratch, uint8_t *scratch_end) {
  ...
  src += 4;
  uint32_t bits_b = __byteswap_ulong(*(uint32_t*)(src_end - 4));
  src_end -= 4;
...
int Kraken_DecodeBytes(
  ...
  int *decoded_size, size_t output_size, bool force_memmove, uint8_t *scratch, uint8_t *scratch_end) {
  const uint8_t *src_org = src;
  int src_size, dst_size;
...
void CombineScaledOffsetArrays(
  int *offs_stream, size_t offs_stream_size, int scale, const uint8_t *low_bits) {
...
bool Kraken_ReadLzTable(int mode,
                        ...
                        uint8_t *scratch, uint8_t *scratch_end, KrakenLzTable *lztable) {
  uint8_t *out;
  int decode_count, n;
...
bool Kraken_ProcessLzRuns_Type0(KrakenLzTable *lzt, uint8_t *dst, uint8_t *dst_end, uint8_t *dst_start) {
...
// Note: may access memory out of bounds on invalid input.
bool Kraken_ProcessLzRuns_Type1(KrakenLzTable *lzt, uint8_t *dst, uint8_t *dst_end, uint8_t *dst_start) {
  const uint8_t *cmd_stream = lzt->cmd_stream, 
             *cmd_stream_end = cmd_stream + lzt->cmd_stream_size;
  const int *len_stream = lzt->len_stream;
  const int *len_stream_end = lzt->len_stream + lzt->len_stream_size;
...
bool Kraken_ProcessLzRuns(int mode, uint8_t *dst, int dst_size, int offset, KrakenLzTable *lztable) {
  uint8_t *dst_end = dst + dst_size;
  ...
  while (dst_end - dst != 0) {
    ...
    if (!(chunkhdr & 0x800000)) {
      ...
      src_used = Kraken_DecodeBytes(
        &out, src, src_end, &written_bytes, dst_count, false, scratch, scratch_end);
...
bool Leviathan_ReadLzTable(int chunk_type,
                           const uint8_t *src, const uint8_t *src_end,
                           uint8_t *dst, int dst_size, int offset,
                           uint8_t *scratch, uint8_t *scratch_end, LeviathanLzTable *lztable) {
...
struct LeviathanModeRaw {
  ...
  inline LeviathanModeRaw(LeviathanLzTable *lzt, uint8_t *dst_start) : lit_stream(lzt->lit_stream[0]) {
  }
  
  inline bool CopyLiterals(
    uint32_t cmd, uint8_t *&dst, const int *&len_stream,
  ...
  inline void CopyFinalLiterals(uint32_t final_len, uint8_t *&dst, size_t last_offset) {
    if (final_len >= 64) {
      do {
        COPY_64_BYTES(dst, lit_stream);
        dst += 64, lit_stream += 64, final_len -= 64;
...
struct LeviathanModeSub {
  ...
  inline LeviathanModeSub(LeviathanLzTable *lzt, uint8_t *dst_start) : lit_stream(lzt->lit_stream[0]) {
  }
...
struct LeviathanModeLamSub {
  ...
  inline LeviathanModeLamSub(LeviathanLzTable *lzt, uint8_t *dst_start) 
...
struct LeviathanModeSubAnd3 {
  ...
  inline LeviathanModeSubAnd3(LeviathanLzTable *lzt, uint8_t *dst_start) {
    for (size_t i = 0; i != NUM; i++)
      lit_stream[i] = lzt->lit_stream[(-(intptr_t)dst_start + i) & MASK];
  }
  inline bool CopyLiterals(uint32_t cmd, uint8_t *&dst, const int *&len_stream, uint8_t *match_zone_end, size_t last_offset) {
    uint32_t lit_cmd = cmd & 0x18;
...
struct LeviathanModeSubAndF {
  ...
  inline LeviathanModeSubAndF(LeviathanLzTable *lzt, uint8_t *dst_start) {
    for(size_t i = 0; i != NUM; i++)
      lit_stream[i] = lzt->lit_stream[(-(intptr_t)dst_start + i) & MASK];
  }
  inline bool CopyLiterals(uint32_t cmd, uint8_t *&dst, const int *&len_stream, uint8_t *match_zone_end, size_t last_offset) {
...
struct LeviathanModeO1 {
  ...
  inline LeviathanModeO1(LeviathanLzTable *lzt, uint8_t *dst_start) {
...
bool Leviathan_ProcessLz(LeviathanLzTable *lzt, uint8_t *dst,
                         uint8_t *dst_start, uint8_t *dst_end, uint8_t *window_base)
{
  const uint8_t *cmd_stream = lzt->cmd_stream,
              *cmd_stream_end = cmd_stream + lzt->cmd_stream_size;
...
bool Leviathan_ProcessLzRuns(int chunk_type, uint8_t *dst, int dst_size, int offset, LeviathanLzTable *lzt) {
...
int Mermaid_DecodeFarOffsets(
  const uint8_t *src, const uint8_t *src_end, uint32_t *output, size_t output_size, int64_t offset
) {
...
bool Mermaid_ReadLzTable(int mode,
                         const uint8_t *src, const uint8_t *src_end,
                         uint8_t *dst, int dst_size, int64_t offset,
                         uint8_t *scratch, uint8_t *scratch_end, MermaidLzTable *lz) {
  uint8_t *out;
  int decode_count, n;
  ...
  if (off16_count == 0xffff) {
    ...
    Mermaid_CombineOffs16((uint16_t*)lz->off16_stream, off16_lo_count, off16_lo, off16_hi);
  } else {
...
const uint8_t *Mermaid_Mode0(uint8_t *dst, size_t dst_size, uint8_t *dst_ptr_end, uint8_t *dst_start,
                          const uint8_t *src_end, MermaidLzTable *lz, int32_t *saved_dist, size_t startoff) {
  ...
  while (cmd_stream < cmd_stream_end) {
    ...
    if (cmd >= 24) {
      ...
      off16_stream = (uint16_t*)((uintptr_t)off16_stream + (use_distance & 2));
...
const uint8_t *Mermaid_Mode1(uint8_t *dst, size_t dst_size, uint8_t *dst_ptr_end, uint8_t *dst_start,
                         const uint8_t *src_end, MermaidLzTable *lz, int32_t *saved_dist, size_t startoff) {
  const uint8_t *dst_end = dst + dst_size;
  const uint8_t *cmd_stream = lz->cmd_stream;
  const uint8_t *cmd_stream_end = lz->cmd_stream_end;
...
bool Mermaid_ProcessLzRuns(int mode,
                           const uint8_t *src, const uint8_t *src_end,
                           uint8_t *dst, size_t dst_size, uint64_t offset, uint8_t *dst_end,
                           MermaidLzTable *lz) {
...
int Mermaid_DecodeQuantum(uint8_t *dst, uint8_t *dst_end, uint8_t *dst_start,
                          const uint8_t *src, const uint8_t *src_end,
...
size_t Bitknit_Decode(
  ...
  uint8_t *dst_start, BitknitState *bk);
...
bool Kraken_DecodeStep(struct KrakenDecoder *dec,
                       uint8_t *dst_start, int offset, size_t dst_bytes_left_in,
                       uint8_t *src, size_t src_bytes_left) {
  const uint8_t *src_in = src;
...
int Kraken_Decompress(uint8_t *src, size_t src_len, uint8_t *dst, size_t dst_len) {
