...
static void inline LznaRenormalize(LznaBitReader *tab) {
...
static uint32_t inline LznaReadBit(LznaBitReader *tab) {
  int r = tab->bits_a & 1;
  tab->bits_a >>= 1;
  LznaRenormalize(tab);
  return r;
...
static uint32_t inline LznaReadNBits(LznaBitReader *tab, int bits) {
...
static uint32_t inline LznaReadNibble(LznaBitReader *tab, LznaNibbleModel *model) {
...
static uint32_t inline LznaRead3bit(LznaBitReader *tab, Lzna3bitModel *model) {
  __m128i t, t0, c0;
  uint32_t bitindex;
  uint32_t start, end;
  uint64_t x = tab->bits_a;
...
static uint32_t inline LznaRead1Bit(LznaBitReader *tab, LznaBitModel *model, int nbits, int shift) {
  uint64_t q;
  int magn = 1 << nbits;
...
static uint32_t inline LznaReadFarDistance(LznaBitReader *tab, LznaState *lut) {
...
static uint32_t inline LznaReadNearDistance(LznaBitReader *tab, LznaState *lut, LznaNearDistModel *model) {
...
static uint32_t inline LznaReadLength(LznaBitReader *tab, LznaLongLengthModel *model, int64_t dst_offs) {
...
int LZNA_DecodeQuantum(uint8_t *dst, uint8_t *dst_end, uint8_t *dst_start,
  ...
  while (dst < dst_end) {
    ...
    if (LznaRead1Bit(&tab, &lut->is_literal[(dst_offs & 7) + 8 * state], 13, 5)) {
      ...
      x = (x << 4) + LznaReadNibble(&tab, ((match_val >> 4) != x) ? &model->nomatch[x] : 
